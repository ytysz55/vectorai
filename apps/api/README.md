...
- `POST /v1/jobs` — raw PNG/JPEG upload, `202` with a [versioned job status](../../schemas/local-job.schema.json). Optional `X-VectorAI-Filename` normalizes PNG/JPEG names to `input.png` or `input.jpg`; `X-VectorAI-Mode` accepts **`faithful`, `geometric`, `minimal`, or `stroke`**. Faithful and minimal invoke distinct pinned E5 optimizer profiles (`--optimizer-profiles` overrides the packaged copy); geometric uses the fast topology-checked baseline, and stroke uses the line-art pipeline. Optimizer fallback is reported as `degraded`, not presented as an optimized result. Maximum request body: 32 MiB, enforced while streaming. Optional `Idempotency-Key` (visible ASCII, 1–128 bytes) replays the original job for the same input/mode/normalized extension, even if it has completed; reuse with different data returns typed `409 IDEMPOTENCY_CONFLICT`.
...
- `GET /v1/jobs/{job_id}/artifacts/{artifact_name}` — allowlisted artifacts are served only after a validated `success`, `degraded`, or `needs_review` result. In-progress/failed/canceled jobs do not publish files. No arbitrary paths or symlink artifacts are served. The legacy demo endpoint still serves its own output via this path.
- `GET /v1/jobs/{job_id}/preview.svg` — inline, sandboxed SVG for an **E7 validated job only**; enforces the same artifact-path guard and is not a PDF or external baseline renderer claim. Legacy demo output cannot use this route. Multicolor E7 `scene.json` includes an opt-in, budgeted `inspection_overlay` with model vertices, face IDs and canonical **graph** edge provenance; E6/benchmark jobs and stroke jobs do not promise that overlay. It is not the fitted SVG path.
- `GET /health` — readiness and `local_only: true`.

The legacy synchronous `POST /v1/vectorize` stays available for the E3 demo; it is **not** the E7 job API. Its `faithful`/`minimal` labels still use the default geometric pipeline, unlike the four working `/v1/jobs` modes. The E7 UI uses job polling and cancellation; do not use the legacy route to benchmark optimizer modes.
